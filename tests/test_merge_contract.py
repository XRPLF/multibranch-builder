...
from xrpld_compose import merge
from xrpld_compose.merge import (
...
@patch("xrpld_compose.merge.shutil.which", return_value="/usr/bin/claude")
def test_ai_resolve_invokes_claude_with_budget_and_timeout(mock_which, tmp_path):
    ...
    with patch.object(merge, "_run", fake_run), patch("xrpld_compose.merge.subprocess.run") as claude:
...
def test_ai_resolve_false_on_timeout(mock_which, tmp_path):
    with patch.object(merge, "_run", lambda cmd, cwd, check=True: _completed(stdout="a.cpp")), \
         patch("xrpld_compose.merge.subprocess.run", side_effect=subprocess.TimeoutExpired("claude", 1)):
...
def test_ai_resolve_false_when_markers_remain(mock_which, tmp_path):
    ...
    with patch.object(merge, "_run", lambda cmd, cwd, check=True: _completed(stdout="a.cpp")), \
         patch("xrpld_compose.merge.subprocess.run", return_value=_completed()):
        assert ai_resolve(str(tmp_path), "b", "develop") is False


@patch("xrpld_compose.merge.shutil.which", return_value=None)
def test_ai_resolve_false_without_claude_cli(mock_which):
    with patch.object(merge, "_run", lambda cmd, cwd, check=True: _completed(stdout="a.cpp")), \
         patch("xrpld_compose.merge.subprocess.run") as claude:
