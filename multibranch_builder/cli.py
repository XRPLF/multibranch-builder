"""`multibranch-builder` — compose, build, push, manifest."""
...
def cmd_build(args: argparse.Namespace) -> int:
    ...
    try:
        if args.tree:
            ...
            if not manifest_path.is_file():
                sys.exit(f"[multibranch-builder] {manifest_path} not found — run `multibranch-builder compose` first")
            ...
            if not args.src:
                sys.exit("[multibranch-builder] one of --tree or --src is required")
    ...
    if record["status"] != SUCCESS:
        sys.exit(f"[multibranch-builder] build {record['build_id']} finished {record['status']}")
    print(record["image"])
    return 0
